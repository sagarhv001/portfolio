  ...
  useEffect(() => {
    ...
    class App {
      ...
      init() {
        ...
        // ponytail: the canvas sits behind the page content so it never receives a real mousedown,
        // and the dock is in a sibling tree where a ref can't reach — a window event is the seam
        this.onWarp = () => {
          this.onMouseDown();
          clearTimeout(this.warpTimer);
          this.warpTimer = setTimeout(() => this.onMouseUp(), 700);
        };
        window.addEventListener('hyperspeed:warp', this.onWarp);
      ...
      dispose() {
        ...
        clearTimeout(this.warpTimer);
        if (this.onWarp) {
          window.removeEventListener('hyperspeed:warp', this.onWarp);
        }
        if (this.container) {
    ...
    // ponytail: StrictMode remounts dispose the renderer (forceContextLoss) while
    // loadAssets is still pending — skip init if this app is no longer the live one.
